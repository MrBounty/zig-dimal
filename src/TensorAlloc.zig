...
pub fn TensorAlloc(
    ...
    return struct {
        ...
        /// RHS must have the same shape as self, or total == 1 (broadcast).
        pub inline fn sub(self: *const Self, rhs: anytype) TensorAlloc(
            T,
            dims.argsOpt(),
            sh.finerScales(Self, @TypeOf(rhs)).argsOpt(),
            shape,
        ...
        pub inline fn mul(self: *const Self, rhs: anytype) TensorAlloc(
            T,
            ...
            const SelfNorm = TensorAlloc(T, dims.argsOpt(), sh.finerScales(Self, RhsType).argsOpt(), shape);
            const RhsNorm = TensorAlloc(T, RhsType.dims.argsOpt(), sh.finerScales(Self, RhsType).argsOpt(), shape);
        ...
        /// Shape {1} RHS is automatically broadcast across all elements.
        pub inline fn div(self: *const Self, rhs: anytype) TensorAlloc(
            T,
        ...
        pub inline fn pow(self: *const Self, comptime exp: comptime_int) TensorAlloc(
            T,
        ...
        pub inline fn sqrt(self: *const Self) TensorAlloc(
            T,
            dims.div(2).argsOpt(),
            scales.argsOpt(),
            shape,
        ...
        pub inline fn contract(
            ...
            const SelfNorm = TensorAlloc(T, dims.argsOpt(), sh.finerScales(Self, RhsType).argsOpt(), shape);
            const OtherNorm = TensorAlloc(T, RhsType.dims.argsOpt(), sh.finerScales(Self, RhsType).argsOpt(), RhsType.shape);
        ...
        pub inline fn cross(self: *const Self, rhs: anytype) TensorAlloc(
            T,
            dims.add(@TypeOf(rhs).dims).argsOpt(),
            sh.finerScales(Self, @TypeOf(rhs)).argsOpt(),
            &.{3},
        ...
        pub inline fn product(self: *const Self) TensorAlloc(
            T,
...
// test "Scalar MulBy" {
//     const Meter = TensorAlloc(i128, .{ .L = 1 }, .{}, &.{1});
//     const Second = TensorAlloc(f32, .{ .T = 1 }, .{}, &.{1});
//
...
//     const KiloMeter = TensorAlloc(f32, .{ .L = 1 }, .{ .L = .k }, &.{1});
//     const KiloGram = TensorAlloc(f32, .{ .M = 1 }, .{ .M = .k }, &.{1});
//
...
//     const Meter = TensorAlloc(i128, .{ .L = 1 }, .{ .L = .k }, &.{1});
//     const Second = TensorAlloc(f64, .{ .T = 1 }, .{}, &.{1});
//     const KmSec = TensorAlloc(i64, .{ .L = 1, .T = 1 }, .{ .L = .k }, &.{1});
//     const KmSec_f = TensorAlloc(f32, .{ .L = 1, .T = 1 }, .{ .L = .k }, &.{1});
//
//     const d = Meter.splat(3);
//     const t = Second.splat(4);
//
...
//     const Meter = TensorAlloc(i128, .{ .L = 1 }, .{ .L = .n }, &.{1});
//     const Second = TensorAlloc(f32, .{ .T = 1 }, .{}, &.{1});
//     const d = Meter.splat(3);
//     const t = Second.splat(4);
...
//     const DimLess = TensorAlloc(i128, .{}, .{}, &.{1});
//     const Meter = TensorAlloc(i128, .{ .L = 1 }, .{}, &.{1});
...
//
// test "Scalar Sqrt" {
//     const MeterSquare = TensorAlloc(i128, .{ .L = 2 }, .{}, &.{1});
//     const MeterSquare_f = TensorAlloc(f64, .{ .L = 2 }, .{}, &.{1});
//
...
//     const Dimless = TensorAlloc(i128, .{}, .{}, &.{1});
//     const KiloMetre = TensorAlloc(i128, .{ .L = 1 }, .{ .L = .k }, &.{1});
//
...
//     const KiloMeter = TensorAlloc(i128, .{ .L = 1 }, .{ .L = .k }, &.{1});
//     const Meter = TensorAlloc(i128, .{ .L = 1 }, .{}, &.{1});
//     const CentiMeter = TensorAlloc(i128, .{ .L = 1 }, .{ .L = .c }, &.{1});
//
...
//     const Hour = TensorAlloc(i128, .{ .T = 1 }, .{ .T = .hour }, &.{1});
//     const Minute = TensorAlloc(i128, .{ .T = 1 }, .{ .T = .min }, &.{1});
//     const Second = TensorAlloc(i128, .{ .T = 1 }, .{}, &.{1});
//
...
//
// test "Scalar Format" {
//     const MeterPerSecondSq = TensorAlloc(f32, .{ .L = 1, .T = -2 }, .{ .T = .n }, &.{1});
//     const Meter = TensorAlloc(f32, .{ .L = 1 }, .{}, &.{1});
//
...
// test "Scalar Abs" {
//     const Meter = TensorAlloc(i128, .{ .L = 1 }, .{}, &.{1});
//     const MeterF = TensorAlloc(f32, .{ .L = 1 }, .{}, &.{1});
//
...
//     const Foot = TensorAlloc(f64, .{ .L = 1 }, .{ .L = .ft }, &.{1});
//     const Meter = TensorAlloc(f64, .{ .L = 1 }, .{}, &.{1});
//     const Inch = TensorAlloc(f64, .{ .L = 1 }, .{ .L = .inch }, &.{1});
//
...
//     const Pound = TensorAlloc(f64, .{ .M = 1 }, .{ .M = .lb }, &.{1});
//     const Ounce = TensorAlloc(f64, .{ .M = 1 }, .{ .M = .oz }, &.{1});
//
...
//     const Meter4 = TensorAlloc(f32, .{ .L = 1 }, .{}, &.{4});
...
//
// test "Vector format" {
//     const MeterPerSecondSq = TensorAlloc(f32, .{ .L = 1, .T = -2 }, .{ .T = .n }, &.{3});
//     const KgMeterPerSecond = TensorAlloc(f32, .{ .M = 1, .L = 1, .T = -1 }, .{ .M = .k }, &.{3});
//
//     const accel = MeterPerSecondSq.splat(9.81);
...
//     const Meter3 = TensorAlloc(i32, .{ .L = 1 }, .{}, &.{3});
//
...
//     const Meter3 = TensorAlloc(i32, .{ .L = 1 }, .{}, &.{3});
//     const Second1 = TensorAlloc(i32, .{ .T = 1 }, .{}, &.{1});
//
...
//     const KiloMeter3 = TensorAlloc(i32, .{ .L = 1 }, .{ .L = .k }, &.{3});
//     const Meter3 = TensorAlloc(i32, .{ .L = 1 }, .{}, &.{3});
//
...
//
// test "Vector Length" {
//     const MeterInt3 = TensorAlloc(i32, .{ .L = 1 }, .{}, &.{3});
//     const MeterFloat3 = TensorAlloc(f32, .{ .L = 1 }, .{}, &.{3});
//
...
//     const Meter3 = TensorAlloc(f32, .{ .L = 1 }, .{}, &.{3});
//     const KiloMeter3 = TensorAlloc(f32, .{ .L = 1 }, .{ .L = .k }, &.{3});
//
...
//     const Meter3 = TensorAlloc(f32, .{ .L = 1 }, .{}, &.{3});
//     const KiloMeter1 = TensorAlloc(f32, .{ .L = 1 }, .{ .L = .k }, &.{1});
//
...
//
//     const Meter1 = TensorAlloc(f32, .{ .L = 1 }, .{}, &.{1});
...
//     const Meter3 = TensorAlloc(f32, .{ .L = 1 }, .{}, &.{3});
//     const Newton3 = TensorAlloc(f32, .{ .M = 1, .L = 1, .T = -2 }, .{}, &.{3});
//
...
//     const A = TensorAlloc(f32, .{}, .{}, &.{ 2, 3 });
//     const B = TensorAlloc(f32, .{}, .{}, &.{ 3, 2 });
//
//     const a = A{ .data = .{ 1, 2, 3, 4, 5, 6 } };
//     const b = B{ .data = .{ 7, 8, 9, 10, 11, 12 } };
//
//     const c = a.contract(b, 1, 0);
//     try std.testing.expectEqual(58, c.data[TensorAlloc(f32, .{}, .{}, &.{ 2, 2 }).idx(.{ 0, 0 })]);
//     try std.testing.expectEqual(64, c.data[TensorAlloc(f32, .{}, .{}, &.{ 2, 2 }).idx(.{ 0, 1 })]);
//     try std.testing.expectEqual(139, c.data[TensorAlloc(f32, .{}, .{}, &.{ 2, 2 }).idx(.{ 1, 0 })]);
//     try std.testing.expectEqual(154, c.data[TensorAlloc(f32, .{}, .{}, &.{ 2, 2 }).idx(.{ 1, 1 })]);
// }
//
...
//     const DimLess3 = TensorAlloc(i32, .{}, .{}, &.{3});
...
//     const Mat3x3 = TensorAlloc(f32, .{}, .{}, &.{ 3, 3 });
...
//     const T1 = TensorAlloc(f32, .{}, .{}, &.{3});
//     const T2 = TensorAlloc(f32, .{}, .{}, &.{ 3, 4 });
//     const T3 = TensorAlloc(f32, .{}, .{}, &.{ 2, 3, 4 });
//
